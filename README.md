...
Mesen: If using Mesen-S, open the Debugger window and select `File > Import/Export > Import Labels` or `File > Workspace > Import labels...`. Select the `Mesen.msl` file to add labels to the debugger view and trace logs. If using Mesen2, you can drag the `Mesen.msl` file directly into the debugger window to load it.
